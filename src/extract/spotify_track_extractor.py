    ...
    for attempt in range(max_retries):
        try:
            ...
            if hasattr(e, 'response') and e.response and e.response.status_code == 429:
                ...
                if retry_after:
                    try:
                        ...
                        logger.warning(f"Rate limited. API suggests waiting {wait_time} seconds (from Retry-After header).")
                    except (ValueError, TypeError):
                        wait_time = base_delay * (2 ** attempt) + random.uniform(0, 1)
                        logger.warning(f"Rate limited (attempt {attempt + 1}/{max_retries}). Waiting {wait_time:.1f} seconds...")
                else:
            ...
            if attempt < max_retries - 1:
                wait_time = base_delay * (2 ** attempt) + random.uniform(0, 1)
                logger.warning(f"Connection/timeout error (attempt {attempt + 1}/{max_retries}). Waiting {wait_time:.1f} seconds...")
                time.sleep(wait_time)
                continue
            else:
                logger.error(f"Max retries exceeded for connection error: {url}")
                return None
